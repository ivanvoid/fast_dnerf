...
def render(H, W, K, timestep,chunk=1024*32, rays=None, c2w=None, ndc=True,near=0., far=1., use_viewdirs=False, c2w_staticcam=None, **kwargs):
    ...
    timestep = timestep * torch.ones_like(rays_d[...,:1])
    rays = torch.cat([rays_o, rays_d, near, far, timestep], -1)
    if use_viewdirs:
...
def render_rays(ray_batch,
    ...
    raw = network_query_fn(pts, viewdirs, timestep, network_fn)
    # raw = run_network(pts, viewdirs, timestep, network_fn)
...
#         raw = run_network(pts, fn=run_fn)
        raw = network_query_fn(pts, viewdirs, timestep, run_fn)
